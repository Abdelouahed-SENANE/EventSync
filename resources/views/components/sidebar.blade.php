<aside class="flex flex-col z-50 w-[260px] h-screen fixed top-0 left-0 px-5 py-8 shadow-md shadow-slate-300/20 bg-slate-200 border-r rtl:border-r-0 rtl:border-l">
    <a href="#">
        <img class="w-auto h-10" src="{{ asset('assets/images/logo.png') }}" alt="Logo">
    </a>
    ...
    <div class="flex flex-col justify-between flex-1 mt-14">
        <nav class="-mx-3 space-y-6">
            <div class="space-y-3">
                ...
                <a class="flex items-center px-3 py-2 text-teal-600 transition-colors duration-300 transform rounded-lg hover:bg-teal-600/90 hover:text-white sidebar-link" href="/admin-panel">
            ...
            <div class="space-y-3">
                ...
                <a class="flex items-center px-3 py-2 text-teal-600 transition-colors duration-300 transform rounded-lg hover:bg-teal-600/90 hover:text-white sidebar-link" href="{{ route('admin.users') }}">
                ...
                <a class="flex items-center px-3 py-2 text-teal-600 transition-colors duration-300 transform rounded-lg hover:bg-teal-600/90 hover:text-white sidebar-link" href="{{ route('admin.events') }}">
                    <i class="fa-solid fa-calendar text-[20px]"></i>
                    <span class="mx-2 text-sm font-medium">Events</span>
                </a>
                <a class="flex items-center px-3 py-2 text-teal-600 transition-colors duration-300 transform rounded-lg hover:bg-teal-600/90 hover:text-white sidebar-link" href="{{ route('admin.categories') }}">
                    <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                        <path fill-rule="evenodd" d="M8 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8Zm-2 9a4 4 0 0 0-4 4v1c0 1.1.9 2 2 2h8a2 2 0 0 0 2-2v-1a4 4 0 0 0-4-4H6Zm7.3-2a6 6 0 0 0 0-6A4 4 0 0 1 20 8a4 4 0 0 1-6.7 3Zm2.2 9a4 4 0 0 0 .5-2v-1a6 6 0 0 0-1.5-4H18a4 4 0 0 1 4 4v1a2 2 0 0 1-2 2h-4.5Z" clip-rule="evenodd"/>
                    </svg>
                ...
                <div class="w-full h-full p-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-1">
                            <img src="{{asset('storage/' . Auth::user()->picture)}}" alt="User Picture" class="h-10 w-10 rounded-full">
                            <div>
                                <h4 class="text-[15px]">{{ Auth::user()->name }}</h4>
                                <span class="text-teal-600text-[13px]">{{ Auth::user()->email }}</span>
                            </div>
                        </div>
                        <div class="text-gray-500 relative">
                            ...
                            <div class="p-2 bg-white shadow-md rounded-md w-[150px] hidden absolute bottom-[140%] left-[0px]" id="dropdown">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button role="menuitem" class="flex items-center px-3 py-2 text-gray-700 transition-colors duration-300 transform rounded-lg w-full hover:bg-teal-600 hover:text-white">
